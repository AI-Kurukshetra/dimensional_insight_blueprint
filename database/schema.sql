...
-- =====================================================================
-- RBAC compatibility layer for API permission middleware
-- =====================================================================
alter table if exists public.roles
  add column if not exists name text;

update public.roles
set name = coalesce(name, role_name, role_slug)
where name is null;
...
set name = coalesce(name, permission_name, permission_key)
where name is null;

-- Dedicated activity stream for RBAC action tracking
...
-- Seed required roles: admin, executive, doctor, analyst, viewer
insert into public.roles (organization_id, role_name, role_slug, name, description, is_system)
select
  o.id,
  initcap(r.role_slug),
  r.role_slug,
...
insert into public.permissions (organization_id, permission_key, permission_name, name, description, category)
select
  o.id,
  p.permission_key,
  p.permission_name,
...
cross join (
  values
    ('manage.organizations', 'Manage Organizations', 'Create/update organization configuration', 'admin'),
    ('manage.facilities', 'Manage Facilities', 'Create/update facilities', 'admin'),
    ('manage.users', 'Manage Users', 'Invite and manage users', 'admin'),
    ('view.all.dashboards', 'View All Dashboards', 'Access all dashboard modules', 'admin'),
    ('manage.integrations', 'Manage Integrations', 'Configure external integrations', 'admin'),
    ('configure.alerts', 'Configure Alerts', 'Manage alert rules and subscriptions', 'admin'),
    ('view.executive.dashboards', 'View Executive Dashboards', 'Executive KPI and board views', 'executive'),
    ('view.financial.reports', 'View Financial Reports', 'Access financial reporting', 'executive'),
    ('view.operational.analytics', 'View Operational Analytics', 'Access operations analytics', 'executive'),
    ('export.reports', 'Export Reports', 'Export report files', 'executive'),
    ('view.clinical.dashboards', 'View Clinical Dashboards', 'Access clinical dashboards', 'doctor'),
    ('view.patient.data', 'View Patient Data', 'Read patient-level data', 'doctor'),
    ('view.treatment.outcomes', 'View Treatment Outcomes', 'Read treatment outcomes', 'doctor'),
    ('create.dashboards', 'Create Dashboards', 'Create dashboard definitions', 'analyst'),
    ('run.analytics.queries', 'Run Analytics Queries', 'Execute analytics workloads', 'analyst'),
    ('generate.reports', 'Generate Reports', 'Generate reports from analytics', 'analyst'),
    ('export.datasets', 'Export Datasets', 'Export analytical datasets', 'analyst'),
    ('read.only.dashboards', 'Read Only Dashboards', 'View dashboards in read-only mode', 'viewer')
...
join matrix m
  on m.role_slug = r.role_slug
 and m.permission_key = p.permission_key
